...
Describe 'New-AzMySqlFlexibleServer' {
    ...
    It 'CreateExpanded' -Skip {
        {
            if ($TestMode -eq 'live' -or $TestMode -eq 'record') {
                ...
                $server = New-AzMySqlFlexibleServer -Location $env.location -Name $env.serverName2 -ResourceGroupName $env.resourceGroup -AdministratorUserName mysql_test -AdministratorLoginPassword $password -SkuTier GeneralPurpose -BackupRetentionDay 12 -StorageInMb 102400 -PublicAccess None
    ...
    It 'PublicAccessScenario-FirewallRule' -Skip {
        {
            if ($TestMode -eq 'live' -or $TestMode -eq 'record') {
    ...
    It 'VnetNameScenario-ValidVnetNotExist' -Skip {
        {
            if ($TestMode -eq 'live' -or $TestMode -eq 'record') {
    ...
    It 'VnetNameScenario-InvalidVnet' -Skip {
        if ($TestMode -eq 'live' -or $TestMode -eq 'record') {
            {
                # invalid vnet name
                $InvalidName = "hi/df!@$@#$@"
    ...
    It 'VnetIdScenario-ValidVnet' -Skip {
        {
            if ($TestMode -eq 'live' -or $TestMode -eq 'record') {           
    ...
    It 'VnetIdScenario-ValidVnetNotExist' -Skip {
        {
            if ($TestMode -eq 'live' -or $TestMode -eq 'record') {
    ...
    It 'VnetIdScenario-InvalidVnet' -Skip {
        if ($TestMode -eq 'live' -or $TestMode -eq 'record') {
            {
                # invalid vnet Id
    ...
    It 'SubnetIdScenario-InvalidSubnet' -Skip {
        if ($TestMode -eq 'live' -or $TestMode -eq 'record') {
            {
                # invalid subnet Id
    ...
    It 'VnetSubnetScenario-ValidVnetSubnetNotExist' -Skip {
        {
            if ($TestMode -eq 'live' -or $TestMode -eq 'record') {
    ...
    It 'VnetSubnetScenario-ValidVnetSubnet' -Skip {
        {
            if ($TestMode -eq 'live' -or $TestMode -eq 'record') {
